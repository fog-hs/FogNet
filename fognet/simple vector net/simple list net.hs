...
runNeuron ys (i,xs) | length ys == i = tanh $ sum $ zipWith (*) xs ys
 | otherwise = error", runtime nets!! HLIST!!!!" 

testNeuron = runNeuron [1,1,0] $ neuron [0.1,0.11,0.2]
...
runLayer :: V -> Layer -> V
runLayer xs ns = zipWith runNeuron (replicate (length ns) xs) ns

runNet :: V -> Net -> V
runNet xs (y:ys) = let v = zipWith runNeuron (map return xs) y 
                   in runNet' v ys
 where
  runNet' :: V -> Net -> V
  runNet' xs [] = xs
...
test1 :: V
test1 = runNet (replicate 32 1) egNet 
